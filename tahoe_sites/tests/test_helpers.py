...
from tahoe_sites import helpers
...
class TestHelpers(DefaultsForTestsMixin):
    ...
    def test_get_organization_by_uuid_without_org(self):
        ...
        assert helpers.get_organization_by_uuid(default_site.site_uuid) == self.default_org
    ...
    def test_get_uuid_by_organization_without_org(self):
        ...
        """
        default_site = TahoeSiteUUID.objects.get(organization=self.default_org)
        assert helpers.get_uuid_by_organization(self.default_org) == default_site.site_uuid
    ...
    def test_get_organization_by_uuid_with_org(self):
        ...
        """
        assert helpers.get_organization_by_uuid(self.default_org.edx_uuid) == self.default_org
    ...
    def test_get_uuid_by_organization_with_org(self):
        ...
        """
        assert helpers.get_uuid_by_organization(self.default_org) == self.default_org.edx_uuid
    ...
    def test_get_organizations_for_user_default(self):
        ...
        assert list(helpers.get_organizations_for_user(self.default_user)) == [self.default_org, self.org1]

        # user2 is mapped to two of them, one shared with default_user
        assert list(helpers.get_organizations_for_user(self.user2)) == [self.org1, self.org2]

        # records with inactive user will not be returned
        self.mapping.is_active = False
        self.mapping.save()
        assert list(helpers.get_organizations_for_user(self.default_user)) == [self.default_org]
    ...
    def test_get_organizations_for_user_with_inactive_users(self):
        ...
        self.mapping.is_active = False
        self.mapping.save()
        assert list(helpers.get_organizations_for_user(self.default_user, with_inactive_users=True)) == [
            self.default_org,
            self.org1
        ]
    ...
    def test_get_organizations_for_user_without_admins(self):
        ...
        self.mapping.save()
        assert list(helpers.get_organizations_for_user(self.default_user, without_admins=True)) == [self.default_org]
    ...
    def test_get_users_of_organization(self):
        ...
        assert list(helpers.get_users_of_organization(self.default_org)) == [self.default_user]

        # Org1 is mapped to two users
        assert list(helpers.get_users_of_organization(self.org1)) == [self.default_user, self.user2]

        # inactive users will not be returned
        self.mapping.is_active = False
        self.mapping.save()
        assert list(helpers.get_users_of_organization(self.org1)) == [self.user2]
    ...
    def test_get_users_of_organization_with_inactive_users(self):
        ...
        self.mapping.is_active = False
        self.mapping.save()
        assert list(helpers.get_users_of_organization(self.org1, with_inactive_users=True)) == [
            self.default_user,
            self.user2
        ]
    ...
    def test_is_active_admin_on_organization(self):
        ...
        assert not helpers.is_active_admin_on_organization(user=self.default_user, organization=self.org1)

        self.mapping.is_admin = True
        self.mapping.save()
        assert helpers.is_active_admin_on_organization(user=self.default_user, organization=self.org1)
